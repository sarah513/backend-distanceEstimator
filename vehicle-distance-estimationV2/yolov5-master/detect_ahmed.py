...
@torch.no_grad()
def distance_estimation(label,information,model,weights,results_dir):
...
def run(
    ...
    if webcam:
        ...
        print('detect_ahmed',source)
        dataset = LoadImages(path=myImg, img_size=imgsz, stride=stride, auto=pt)
        bs = 1  # batch_size
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        csvRowList = [frame, x1, y1, x2, y2, scaledX1, scaledY1, scaledX2, scaledY2,label]
                        print(label)
                        inf=label.split(' ')
                        print(inf)
                        data = {
                            ...
                            'xmax': [x2],
                            'ymax': [y2],
                            'scaled_xmin': [scaledX1],
                            'scaled_ymin': [scaledY1],
                            'scaled_xmax': [scaledX2],
                            'scaled_ymax':[scaledY2]
                        }
                        with open('results/data/data.csv', 'a') as csvFile:
                            distance_estimation(inf[0],data,r'F:\4th 2\GP\distance_estmation\vehicle-distance-estimationV2\distance-estimator\models\model@1535477330.json',r'F:\4th 2\GP\distance_estmation\vehicle-distance-estimationV2\distance-estimator\models\model@1535477330.h5',r'F:\4th 2\GP\distance_estmation\vehicle-distance-estimationV2\distance-estimator')
