...
def write_predictions_on_frames(df):
    for idx, row in df.iterrows():
        ...
        if os.path.exists(fp):
            im = cv2.imread(fp)
            string = "{} meters".format(round(row['distance'],2))
...
def generate_video_from_frames():
    ...
    img_array.sort()
    out = cv2.VideoWriter("C:Users/ahmed/OneDrive/Desktop/gp/vehicle-distance-estimationV2/distance-estimator/output.mp4", cv2.VideoWriter_fourcc('m' , 'p' , '4' , 'v'), int(fps), size)
